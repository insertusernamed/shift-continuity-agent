...
export class AgentCoreStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: AgentCoreStackProps) {
    ...
    // Durable operational event history (see src/store/dynamoDbStore.ts).
    //
    // One table, on-demand billing: no streams, no backups, no provisioned
    // capacity, no extra resources. It stores *source events*, not folded state,
    // so the domain remains the single source of truth.
    const shiftEventLog = new dynamodb.Table(this, 'ShiftEventLog', {
      tableName: `${spec.name}-shift-events`,
      partitionKey: { name: 'PK', type: dynamodb.AttributeType.STRING },
      sortKey: { name: 'SK', type: dynamodb.AttributeType.STRING },
      billingMode: dynamodb.BillingMode.PAY_PER_REQUEST,
      // Hackathon environment: default (AWS-owned) encryption is sufficient, and
      // DESTROY keeps `agentcore remove` able to clean up. Routine deploys only
      // *update* the stack, so they never remove stored events.
      removalPolicy: RemovalPolicy.DESTROY,
      deletionProtection: false,
      pointInTimeRecoverySpecification: { pointInTimeRecoveryEnabled: false },
    });

    // `listShifts` is a concrete query, so the store has exactly one index.
    shiftEventLog.addGlobalSecondaryIndex({
    ...
    // Point every runtime at the durable store and grant only the data-plane
    // actions the store actually issues (GetItem, Query, conditional PutItem).
    // This project has a single runtime; the loop keeps it correct if that changes.
    for (const env of this.application.environments.values()) {
      env.runtime.addEnvironmentVariable('SHIFT_STORE', 'dynamodb');
      env.runtime.addEnvironmentVariable('SHIFT_TABLE_NAME', shiftEventLog.tableName);
      shiftEventLog.grant(env.runtime.role, 'dynamodb:GetItem', 'dynamodb:Query', 'dynamodb:PutItem');
    }

    new CfnOutput(this, 'ShiftEventLogTableName', {
      description: 'DynamoDB table holding the append-only operational event log',
      value: shiftEventLog.tableName,
    });
